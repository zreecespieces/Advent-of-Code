...
const exampleSchematic = `467..114..
...*......
..35..633.
......#...
617*......
...
function getPartNumbers(currentLine, previousLine, nextLine) {
    const partNumbers = []
    const symbols=["+", "*", "/", "&", "%", "@", "=", "$", "-", "#"]

    let tempNumber = ""
    let numberStartIndex = undefined
    let surroundedBySymbol = false

    for (let i = 0; i < currentLine.length; i++) {
        if (!isNaN(parseInt(currentLine[i]))) {
            if (!tempNumber) numberStartIndex = i

            const nextIsNumber = !isNaN(parseInt(currentLine[i + 1]))
            const previousWasNumber = !!tempNumber.length
            tempNumber += currentLine[i]

            if ((!previousWasNumber && symbols.includes(currentLine[i - 1])) || (!nextIsNumber && symbols.includes(currentLine[i + 1]))) {
                surroundedBySymbol = true
            } else if (previousLine && (symbols.includes(previousLine[i]) || (!previousWasNumber && symbols.includes(previousLine[i - 1])) || (!nextIsNumber && symbols.includes(previousLine[i + 1])))) {
                surroundedBySymbol = true
            } else if (nextLine && (symbols.includes(nextLine[i]) || (!previousWasNumber && symbols.includes(nextLine[i - 1])) || (!nextIsNumber && symbols.includes(nextLine[i + 1])))) {
                surroundedBySymbol = true
            }

            // handle number being on the very end of the line
            if (currentLine[i + 1] === undefined && tempNumber && surroundedBySymbol) {
                partNumbers.push({startEnd: [numberStartIndex, numberStartIndex + tempNumber.length - 1], value: parseInt(tempNumber)})
            }
        } else {
            if (tempNumber && surroundedBySymbol) {
                partNumbers.push({startEnd: [numberStartIndex, numberStartIndex + tempNumber.length - 1], value: parseInt(tempNumber)})
            }

            tempNumber = ""
            surroundedBySymbol = false
        }
    }
...
function sumPartNumbers(string) {
    const sum = string
        .split("\n")
        .reduce((overallTotal, currentValue, currentIndex, array) => {
            const linePartNumbers = getPartNumbers(currentValue, array[currentIndex - 1], array[currentIndex + 1]).map(p => p.value)
            const lineTotal = linePartNumbers.reduce((lineTotal, partNumber) => lineTotal + partNumber, 0);
            return overallTotal + lineTotal;
        }, 0);
    console.log("SUM", sum)
}

function checkPrevNextAdjacent(p, i) {
    const [start, end] = p.startEnd
    let isAdjacent = false

    if (i === start || i === end) isAdjacent = true
    if (i > start && i < end) isAdjacent = true
    if (start === i + 1) isAdjacent = true
    if (end === i - 1 ) isAdjacent = true

    return isAdjacent
}

function getGears(currentLineString, currentLinePartNumbers, previousLinePartNumbers, nextLinePartNumbers) {
    const gearRatios = []

    for (let i = 0; i < currentLineString.length; i++) {
        if (currentLineString[i] !== "*") continue
        
        const allAdjacent = [...previousLinePartNumbers, ...currentLinePartNumbers, ...nextLinePartNumbers].filter((p) => checkPrevNextAdjacent(p, i))
        if (allAdjacent.length !== 2) continue

        const ratio = allAdjacent[0].value * allAdjacent[1].value
...
function sumGearRatios(string) {
    const lines = string.split("\n")
    const partNumbers = lines.map((val, index, array) => getPartNumbers(val, array[index - 1], array[index + 1]))
    const gears = lines.map((val, index) => getGears(val, partNumbers[index], partNumbers[index - 1], partNumbers[index + 1])).flatMap(v => v, [])
    console.log("GEARS", gears.reduce((total, currentValue) => total + currentValue, 0))
}

sumPartNumbers(actualSchematic)
sumGearRatios(actualSchematic)