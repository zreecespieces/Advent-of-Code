const actualGames = `Game 1: 2 green, 6 blue, 7 red; 12 green, 6 blue, 3 red; 5 red, 18 green, 4 blue
...
const exampleGames = `Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
function parseGame(string) {
    const gameWithID = string.split(":")[0]
    const id = parseInt(gameWithID.split(" ")[1])
    const rounds = string.replace(gameWithID + ": ", "").split(";")
    let possible = true
...
function parseGame2(string) {
    const gameWithID = string.split(":")[0]
    const rounds = string.replace(gameWithID + ": ", "").split(";")
    const max = { blue: 1, green: 1, red: 1 }
    ...
    for (let i = 0; i < rounds.length; i++) {
        ...
        for (const cube of cubes) {
            const [number, color] = cube.trim().split(" ")
            if (number > max[color]) max[color] = parseInt(number)
        }
    }

    return Object.values(max).reduce((total, currentValue) => total * currentValue, 1)
}

function calculateSum(string) {
    return string.split("\n").reduce((total, currentValue) => total + parseGame2(currentValue), 0)
}

console.log(calculateSum(actualGames))