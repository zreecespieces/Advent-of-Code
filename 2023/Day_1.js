const calibrationDocument = `8eight1
98126
...
const testString = "six3zbhvrfhsevennine"
...
const stringNumberKeys = Object.keys(stringNumbers)
...
function getFirstLastDigits(string) {
    ...
    while (pointer2 >= 0) {
        const char1 = string[pointer1]
        const char2 = string[pointer2]

        slice1 += char1
        slice2 = char2 + slice2

        const foundNumberString1 = stringNumberKeys.find(s => slice1.includes(s))
        const foundNumberString2 = stringNumberKeys.find(s => slice2.includes(s))

        if (!firstDigit && foundNumberString1) {
            firstDigit = stringNumbers[foundNumberString1]
        }

        if (!lastDigit && foundNumberString2) {
            lastDigit = stringNumbers[foundNumberString2]
        }

        const int1 = parseInt(char1)
        const int2 = parseInt(char2)

        if (!firstDigit && !isNaN(int1)) {
            firstDigit = int1
        }

        if (!lastDigit && !isNaN(int2)) {
            lastDigit = int2
        }
...
function sumCalibrationStrings(calibrationString) {
    const strings = calibrationString.split('\n')

    return strings.reduce((runningTotal, valueAtCurrentIndex) => {
        const twoDigitNumberCombination = getFirstLastDigits(valueAtCurrentIndex)
        return runningTotal + parseInt(twoDigitNumberCombination)
    }, 0)
}

console.log(sumCalibrationStrings(calibrationDocument))
// console.log(getFirstLastDigits(testString))